...
const Header = () => {
  ...
  return (
    <header className='py-6 mb-12 border-b border-orange-500'>
      ...
      {/* Login Modal */}
      {showLoginModal && (
        <LoginModal handleLoginSubmit={handleLoginSubmit} setShowLoginModal={setShowLoginModal} />
      )}

      {/* Signup Modal */}
      {showSignupModal && (
        <SignupModal handleSignupSubmit={handleSignupSubmit} setShowSignupModal={setShowSignupModal} />
      )}
    </header>
  );
};
